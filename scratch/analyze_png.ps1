...
$files = Get-ChildItem -Path $dir -Filter "marc*.png"

foreach ($fileItem in $files) {
    $res = Analyze-Png $fileItem.FullName
    Write-Host "$($res.File.PadRight(24)) -> Total: $($res.Width)x$($res.Height) (ratio $($res.TotalRatio)) | Inner: $($res.InnerWidth)x$($res.InnerHeight) (ratio $($res.InnerRatio)) | Margins: L=$($res.MarginL) R=$($res.MarginR) T=$($res.MarginT) B=$($res.MarginB) | Transp: $($res.TransparentPixels)"
}
